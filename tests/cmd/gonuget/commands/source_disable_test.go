...
// TestSourceDisableCommandStructure validates the source disable command structure
func TestSourceDisableCommandStructure(t *testing.T) {
	console := output.NewTestConsole()
	cmd := commands.NewSourceDisableCommand(console)

	// VR-002: Subcommand Use field must be verb-only (with args allowed)
	if cmd.Use != "disable <NAME>" {
		t.Errorf("Expected Use='disable <NAME>', got '%s'", cmd.Use)
	}
...
// TestSourceDisableFlags validates the flags for source disable command
func TestSourceDisableFlags(t *testing.T) {
	console := output.NewTestConsole()
	cmd := commands.NewSourceDisableCommand(console)
