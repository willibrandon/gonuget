...
**NuGet.Client Reference**:
- `NuGet.Packaging/RuntimeModel/RuntimeGraph.cs` - Core RID graph with BFS expansion
- `NuGet.Packaging/RuntimeModel/RuntimeDescription.cs` - RID descriptor
- `NuGet.Packaging/RuntimeModel/JsonRuntimeFormat.cs` - JSON loading (uses `#import`)
...
**IMPORTANT**: See `M3.13-NUGET-CLIENT-COMPARISON.md` for critical implementation notes on:
- BFS expansion algorithm (required for correct RID ordering)
- 'any' and 'base' foundation RIDs (required for official compatibility)
- Expansion caching (required for performance)
...
// Note: NuGet.Client calls this "InheritedRuntimes" but "Imports" is clearer
...
func LoadDefaultRuntimeGraph() *RuntimeGraph {
    ...
    // Reference: NuGet.Client test runtime.json files
...
// CRITICAL: This ordering determines which assets are selected when multiple RID-specific versions exist.
...
func (g *RuntimeGraph) ExpandRuntime(rid string) []string {
    ...
    // BFS traversal (matches NuGet.Client RuntimeGraph.cs:186-220)
    var result []string
...
func (g *RuntimeGraph) AreCompatible(targetRID, packageRID string) bool {
    ...
    // Check cache
    key := compatKey{target: targetRID, package: packageRID}
    g.cacheMutex.RLock()
...
// (matches NuGet.Client RuntimeGraph.cs:254-298)
...
// LoadFromJSON loads a runtime graph from JSON (matches NuGet.Client JsonRuntimeFormat.cs)
